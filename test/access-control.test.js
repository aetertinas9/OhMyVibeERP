...
test("관리자는 모든 권한을 갖고 부서 계정은 지정된 자기 업무만 갖는다", () => {
  ...
  assert.equal(canAccess(user("purchasing"), PERMISSIONS.INVENTORY_TRANSFER), false);
  assert.equal(canAccess(user("purchasing"), PERMISSIONS.INVENTORY_COUNT), false);
  assert.equal(canAccess(user("logistics"), PERMISSIONS.PURCHASE_RECEIVE), true);
  assert.equal(canAccess(user("logistics"), PERMISSIONS.PURCHASE_RETURN), true);
  assert.equal(canAccess(user("logistics"), PERMISSIONS.SALES_SHIP), true);
  assert.equal(canAccess(user("logistics"), PERMISSIONS.SALES_RETURN), true);
  assert.equal(canAccess(user("logistics"), PERMISSIONS.INVENTORY_TRANSFER), true);
  assert.equal(canAccess(user("logistics"), PERMISSIONS.INVENTORY_COUNT), true);
  ...
  assert.equal(canAccess(user("finance"), PERMISSIONS.FINANCE_SETTLEMENTS), true);
...
test("동적 입출고·급여명세 경로까지 HTTP 요청 권한을 분류한다", () => {
  ...
  assert.equal(permissionForRequest("POST", "/purchase-orders/po-1/return"), PERMISSIONS.PURCHASE_RETURN);
  assert.equal(permissionForRequest("POST", "/sales-orders/so-1/ship"), PERMISSIONS.SALES_SHIP);
  assert.equal(permissionForRequest("POST", "/sales-orders/so-1/return"), PERMISSIONS.SALES_RETURN);
  assert.equal(permissionForRequest("POST", "/inventory/transfers"), PERMISSIONS.INVENTORY_TRANSFER);
  assert.equal(permissionForRequest("POST", "/inventory/counts"), PERMISSIONS.INVENTORY_COUNT);
  assert.equal(permissionForRequest("POST", "/inventory/replenishments"), PERMISSIONS.PURCHASE_ORDERS_CREATE);
  ...
  assert.equal(permissionForRequest("GET", "/reports/vat"), PERMISSIONS.FINANCE_REPORT);
  assert.equal(permissionForRequest("GET", "/settlements"), PERMISSIONS.FINANCE_SETTLEMENTS);
  assert.equal(permissionForRequest("POST", "/settlements/collections"), PERMISSIONS.FINANCE_SETTLEMENTS);
  assert.equal(permissionForRequest("POST", "/settlements/payments"), PERMISSIONS.FINANCE_SETTLEMENTS);
  assert.equal(permissionForRequest("GET", "/accounts"), PERMISSIONS.ACCOUNTS_MANAGE);
  assert.equal(permissionForRequest("POST", "/accounts/usr-1/password"), PERMISSIONS.ACCOUNTS_MANAGE);
  assert.equal(permissionForRequest("POST", "/accounts/usr-1/lock"), PERMISSIONS.ACCOUNTS_MANAGE);
